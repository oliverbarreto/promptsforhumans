...
export function PromptCard({ prompt, onUpdatePrompt }: PromptCardProps) {
  ...
  return (
    <Card className={cn("h-full flex flex-col", prompt.isArchived && "opacity-60")}>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{new Date(prompt.createdAt).toLocaleDateString()}</p>
            </div>
          </div>
      ...
      <CardFooter className="flex justify-between">
        <Button variant="ghost" size="sm" onClick={handleFavoriteToggle}>
          <Heart className={cn("mr-2 h-4 w-4", prompt.isFavorite && "fill-current text-red-500")} />
          {prompt.likes}
        </Button>
